...
func _ready() -> void:
	...
	# warning-ignore:return_value_discarded
	connect("text_entered", Callable(self, "_on_IntegerLineEdit_text_entered"))

func _on_IntegerLineEdit_text_entered(new_text: String) -> void:
	if new_text.is_valid_int() and \
			new_text.to_int() >= minimum \
			and new_text.to_int() <= maximum:
		emit_signal("int_entered", new_text.to_int())
